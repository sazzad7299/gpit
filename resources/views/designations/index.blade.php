...
<div class="card card-custom gutter-b">
  <div class="card-header flex-wrap py-3">
    <div class="card-title">
      <h3 class="card-label">Add designation
    </div>
    <div class="card-toolbar">
         
      <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#designation">
        Add designation
      </button>
    </div>
  </div>
    ...
    <tbody>
      ...
      <td class="text-center">
              <div class="btn-group" role="group" aria-label="Second group">
                  ...
                  <a href="{{ route('designation.edit', $designation->id) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
              </div>
                  <div class="btn-group" role="group" aria-label="Second group">
                    <button class="btn btn-danger" type="button" onclick="deletedesignation({{ $designation->id }})">
                      <i class="fa fa-trash"></i>
                    </button>
                    <form id="delete-form-{{ $designation->id }}" action="{{ route('designation.destroy',$designation->id) }}" method="POST" >
                        @csrf
                        @method('DELETE')
                    </form>
              </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="designationLabel">Add designation</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <i class="ti-close"></i>
          </button>
        </div>
        <div class="modal-body">
            <form action="{{ route('designation.store') }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf
