...
<div class="card card-custom gutter-b">
  <div class="card-header flex-wrap py-3">
    ...
    <div class="card-toolbar">
      ...
      <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#verticaladd">
        Add Category
      </button>
    </div>
  </div>
  ...
  <tbody>
    ...
    @foreach($verticaladds as $key=>$verticaladd)
    <tr>
    <td>{{ $key+1}}</td>
    <td>{{ $verticaladd->link }}</td>
    <td>
      <img  src="{{asset('uploads/verticaladd/'.$verticaladd->image)}}" class="img-thumbnail" width="100" height="100" alt="avatar">
  </td>
    
    <td>
        @if($verticaladd->types == 1) 
    ...
    <td class="text-center">
            <div class="btn-group" role="group" aria-label="Second group">
                ...
                <a href="{{ route('verticaladd.edit', $verticaladd->id) }}" class="btn btn-success"><i class="fa fa-edit"></i></a>
            </div>
                <div class="btn-group" role="group" aria-label="Second group">
                  <button class="btn btn-danger" type="button" onclick="deleteverticaladd({{ $verticaladd->id }})">
                    <i class="fa fa-trash"></i>
                  </button>
                  <form id="delete-form-{{ $verticaladd->id }}" action="{{ route('verticaladd.destroy',$verticaladd->id) }}" method="POST" >
                      @csrf
                      @method('DELETE')
                  </form>
            </div>
...
<!-- modal -->
<div class="modal fade" id="verticaladd" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="verticaladdLabel">Add verticaladd</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <i class="ti-close"></i>
          </button>
        </div>
        <div class="modal-body">
            <form action="{{ route('verticaladd.store') }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf
